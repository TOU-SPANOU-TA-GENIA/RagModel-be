...
class ConversationContext:
    ...
    def _store_instruction(self, content: str):
        ...
        # Multiple patterns for instruction detection
        patterns = [
            # "when i say X, you answer Y"
            (r'when i say ["\']?([^"\']+)["\']?\s*,\s*you (?:will |should )?answer ["\']?([^"\']+)["\']?', 1, 2),
            # "when I say X, respond Y"
            (r'when i say ["\']?([^"\']+)["\']?\s*,\s*respond (?:with )?["\']?([^"\']+)["\']?', 1, 2),
            # "if I say X, you say Y"
            (r'if i say ["\']?([^"\']+)["\']?\s*,\s*you say ["\']?([^"\']+)["\']?', 1, 2),
            # "X means Y" (simpler pattern)
            (r'["\']?([^"\']+)["\']?\s*means\s*["\']?([^"\']+)["\']?', 1, 2),
        ]
        ...
        for pattern, trigger_group, response_group in patterns:
            ...
            if match:
                ...
                if trigger and response:
                    self.user_instructions[trigger] = response
                    logger.info(f"📝 Stored instruction: '{trigger}' -> '{response}'")
                    return
        
        # Fallback: try to extract any quoted phrases
        quoted_pairs = re.findall(r'["\']([^"\']+)["\']', content)
        if len(quoted_pairs) >= 2:
            trigger = quoted_pairs[0].strip()
            response = quoted_pairs[1].strip()
            if trigger and response:
                self.user_instructions[trigger] = response
                logger.info(f"📝 Stored instruction (fallback): '{trigger}' -> '{response}'")
